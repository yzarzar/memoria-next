    ...
    <div className={cn("sticky top-60", className)}>
      ...
      >
        <motion.h1
          ...
          viewport={{ once: false }}
        >
          <ColorfulText text="Create Unforgettable" />{" "}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-blue-500">
            <ColorfulText text="Events" />
          </span>
        </motion.h1>
        <motion.p
          className="mt-6 text-lg leading-8 text-white/90 text-center max-w-2xl mx-auto font-medium"
          whileInView={{
            opacity: [0, 1],
            y: [20, 0],
          }}
          ...
          viewport={{ once: false }}
        >
          Discover and share amazing events happening around you. From tech
          conferences to local meetups, find your next memorable experience.
        </motion.p>
        <div className="relative mt-10 h-[20rem]">
          <motion.div
          ...
          >
            <Link
              href="/event"
              className="relative inline-flex items-center justify-center px-8 py-4 text-lg font-bold text-white transition-all duration-200 bg-gradient-to-r from-purple-600 to-blue-500 rounded-xl hover:scale-105 hover:shadow-[0_0_40px_8px_rgba(147,51,234,0.3)] group"
            >
              Browse Events
              <svg
      ...
      >
        {paths.map((path, idx) => (
          <motion.path
            ...
            pathLength={1}
            style={{
              pathLength: pathLengths[Math.min(idx, pathLengths.length - 1)],
              opacity: pathLengths[Math.min(idx, pathLengths.length - 1)],
            }}
            transition={{
              duration: 0.8,
              ease: "easeInOut"
